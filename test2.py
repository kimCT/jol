...
url = 'http://192.168.1.105:8080/behavior'
cctv_name = 'kim'
...
while True:
    ...
    file_name_path = 'captures/' + timestr + '.jpg'

    time.sleep(5)

    cv2.imwrite(file_name_path, frame) #file_name_path에 캡쳐 저장
    path_list.append(file_name_path)

    #path_list 개수 세기
    path_list_len = len(path_list)
    list_num = path_list_len - 1
    frame_man = output_keypoints(image_path=path_list[list_num], proto_file=protoFile_body_25, weights_file=weightsFile_body_25,
                                 threshold=0.1, model_name=path_list[list_num].split('\\')[-1], BODY_PARTS=BODY_PARTS_BODY_25)
    output_keypoints_with_lines(POSE_PAIRS=POSE_PAIRS_BODY_25, frame=frame_man)


    #100개가 되면 path_list 비우기
    if path_list_len == 100:
        for i in range(0, 100):
            del path_list[i]


    image = {'image': open(file_name_path, 'rb')}
    obj = {"cctv_name": cctv_name, "datetime": timestr}

    response = requests.post(url, files=image, data=obj)
